...
return (
  ...
  <select {...register("courseEdition")} id="courseEdition">
    ...
    {openCourseEditions.map(oce => (
        <option key={oce._id} value={oce._id}>{oce.name} - Inicia: {new Date(oce.startDate).toLocaleDateString("es-ar", {timeZone: "UTC"})} - Hora: {oce.startTime} - Precio en pesos argentinos: {oce.priceARS} - Precio en dólares: {oce.priceUSD}</option>
    ))}
    </select>
